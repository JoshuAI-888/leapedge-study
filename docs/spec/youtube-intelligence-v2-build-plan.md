...
| F31 | Phase-2 gate run: four-video parallel end to end on PGlite with fake transports; kill-and-resume; seed dedupe; migrations idempotent; `0004` refuses while a run is `running`; a Neon branch checked through the pooled endpoint | `tests/e2e-parallel.test.ts`, `docs/gates/phase-2-<date>.json` | 9 |
...
| 1 | gate | F21 | |
| 2 | data (blocking) | F22 → F23 → F22a → F24 → F24b | Owns `database.ts`, `migrations/*`, `repos/*`, `research-store.ts` reads, `env.ts`, `vercel.json`, `package.json`, runbook |
...
| 2 | prices | F25 | Starts after F26; owns `market.ts`, `performance.ts`, `settlement.ts`, `metrics/context.ts`, `0003` |
...
6. **Expand and contract.** The build migrates before the new functions are live, so old code runs against the new schema for minutes. Additive migrations therefore ship freely. **A type change to an existing column ships in its own migration (`0004`) and is applied with the queue paused and drained:** `YTI_QUEUE_PAUSED=true`, zero `running` runs, and `migrate.ts` refuses `0004` while any run is `running`. Changing `payload` to `jsonb` or `lease_until` to `timestamptz` in the same deploy as other work breaks the running code mid-step.
...
| A type change to an existing column lands while the previous deployment is still serving requests | Type flips ship alone in `0004`, applied with `YTI_QUEUE_PAUSED=true` and zero running runs, `migrate.ts` refusing otherwise; fail-fast is one-directional so schema-ahead-of-code is safe (7.3, 7.4) |
...
|---|---|---|
| data (blocking) | F22 → F23 → F22a → F24 → F24b | `database.ts`, `migrations/*`, `repos/*`, `research-store.ts` reads, `env.ts`, `vercel.json`, `package.json`, runbook |
...
| prices | F25 (start after F26) | `market.ts`, `performance.ts`, `settlement.ts`, `metrics/context.ts`, `0003` |

...
const source = {
  ...
  segments: [
    {
      id: "a",
      text: "If SPY breaks below 500, exit. Do not wait for a close.",
      start_seconds: 1,
      end_seconds: 5,
    },
    {
      id: "b",
      text: "QQQ is only a watch, not a buy.",
      start_seconds: 6,
      end_seconds: 8,
    },
  ],
...
const c = {
  thesis_en: "Exit on a break below 500.",
...
test("source selection copies exact evidence and rejects invented IDs/duplicates", () => {
  ...
  assert.throws(() =>
    materializeSelectedClaim(
      { ...c, evidence_segment_ids: ["missing"] },
      source,
    ),
  );
  assert.throws(() =>
    materializeSelectedClaim(
      { ...c, evidence_segment_ids: ["a", "a"] },
      source,
    ),
  );
});
test("source selection cannot validate a price or ticker from an unrelated cue", () => {
  const claim = materializeSelectedClaim(
    { ...c, evidence_segment_ids: ["b"] },
    source,
  );
  assert.ok(
    validateClaim(claim, source).includes(
      "Price is not present verbatim in its evidence.",
    ),
  );
  assert.ok(
    validateClaim(claim, source).includes(
...
test("Range materialization copies original text, clears stale translations and rejects invalid bounds", async () => {
  const { materializeEvidenceRanges } =
    await import("../src/features/youtube-intelligence/evidence-selection.ts");
  const { evidence_segment_ids, ...fields } = c;
  const result = materializeEvidenceRanges(
    { ...fields, evidence_ranges: [{ start_id: "a", end_id: "b" }] },
    source,
  );
  assert.equal(
    result.evidence[0].quote_original,
    source.segments.map((s) => s.text).join(""),
  );
  assert.equal(result.evidence[0].quote_translation_en, "");
  for (const range of [
    { start_id: "unknown", end_id: "b" },
    { start_id: "b", end_id: "a" },
  ])
    assert.throws(() =>
      materializeEvidenceRanges(
        { ...fields, evidence_ranges: [range] },
        source,
      ),
    );
  assert.throws(() =>
